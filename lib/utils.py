...
class S2MC2PolynomialDecayLR(LearningRateSchedule):
    """
    Warmup and polynomial decay learning rate for S2MC2 network.
    ...
    def __init__(self, learning_rate, end_learning_rate, warmup_steps, decay_steps, power):
        super(S2MC2PolynomialDecayLR, self).__init__()
...
class S2MC2MultiEpochsDecayLR(LearningRateSchedule):
    """
    Warmup and multi-steps decay learning rate for S2MC2 network.
    ...
    def __init__(self, learning_rate, warmup_steps, multi_epochs, steps_per_epoch, factor=10):
        super(S2MC2MultiEpochsDecayLR, self).__init__()
        self.warmup_flag = False
        if warmup_steps > 0:
            self.warmup_flag = True
            self.warmup_lr = WarmUpLR(learning_rate, warmup_steps)
    ...
    def construct(self, global_step):
        ...
        if self.warmup_flag:
            ...
            lr = decay_lr
        # print('S2MC2MultiEpochsDecayLR:',lr.dtype)
        return lr
