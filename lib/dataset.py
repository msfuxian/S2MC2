...
try:
    from lib.model_utils.config import config, dataset_config
    from lib.image import color_aug, get_affine_transform, affine_transform
    from lib.image import gaussian_radius, draw_umich_gaussian, draw_msra_gaussian, draw_dense_reg
    from lib.visual import visual_image
except ImportError as import_error:
    print('Import Error: {}, trying append path/S2MC2_ms/lib/../'.format(import_error))
...
class RPC(ds.Dataset):
    """
    Encapsulation class of RPC datast.
    Initialize and preprocess of image for training and testing.

    Args:
        data_dir(str): Path of rpc dataset.
        data_opt(edict): Config info for rpc dataset.
        net_opt(edict): Config info for S2MC2.
        run_mode(str): Training or testing.

    Returns:
        Prepocessed training or testing dataset for S2MC2 network.
    """
    ...
    def init(self, data_dir, keep_res=False):
        ...
        logger.info('Initializing rpc 2019 {} data.'.format(self.run_mode))
        if not os.path.isdir(data_dir):
            raise RuntimeError("Invalid dataset path")
        if self.run_mode != "test":
            self.annot_path = os.path.join(data_dir, 'annotations',
                                           'instances_{}2019.json').format(self.run_mode)
        else:
            self.annot_path = os.path.join(data_dir, 'annotations', 'image_info_test-dev2019.json')
        self.image_path = os.path.join(data_dir, '{}2019').format(self.run_mode)
        logger.info('Image path: {}'.format(self.image_path))
        logger.info('Annotations: {}'.format(self.annot_path))

        self.rpc = coco.COCO(self.annot_path)
        image_ids = self.rpc.getImgIds()
        ...
        self.classs_dict = {}
        cat_ids = self.rpc.loadCats(self.rpc.getCatIds())
        for cat in cat_ids:
        ...
        if self.run_mode != "test":
            ...
            for img_id in image_ids:
                idxs = self.rpc.getAnnIds(imgIds=[img_id])
                if idxs:
    ...
    def transfer_rpc_to_mindrecord(self, mindrecord_dir, file_name="s2mc2.train.mind", shard_num=1):
        """Create MindRecord file by image_dir and anno_path."""
        if not os.path.isdir(mindrecord_dir):
            os.makedirs(mindrecord_dir)
        if os.path.isdir(self.image_path) and os.path.exists(self.annot_path):
            logger.info("Create MindRecord based on RPC dataset")
        else:
        ...
        writer.add_schema(s2mc2_json, "s2mc2_json")

        for img_id in self.images:
            image_info = self.rpc.loadImgs([img_id])
            annos = self.rpc.loadAnns(self.anns[img_id])
            # get image
    ...
    def __getitem__(self, index):
        ...
        file_name = self.rpc.loadImgs(ids=[img_id])[0]['file_name']
    ...
    def create_train_dataset(self, mindrecord_dir, prefix="s2mc2.train.mind", batch_size=1,
...
def modelarts_pre_process():
    ...
    config.rpc_data_dir = config.data_path
    config.mindrecord_dir = config.output_path


def rpc2mindrecord():
    """Convert rpc2019 dataset to mindrecord"""
    dsc = RPC(dataset_config, run_mode="train")
    dsc.init(config.rpc_data_dir)
    dsc.transfer_rpc_to_mindrecord(config.mindrecord_dir, config.mindrecord_prefix, shard_num=8)
