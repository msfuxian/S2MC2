...
from lib.utils import Sigmoid, GradScale
from lib.utils import FocalLoss, RegLoss
from lib.decode import DetectionDecode
from lib.hourglass import Convolution, Residual, Kp_module
...
class GatherDetectionFeatureCell(nn.Cell):
    ...
    Args:
        net_config: The config info of S2MC2 network.
...
class S2MC2LossCell(nn.Cell):
    ...
    """
    def __init__(self, net_config):
        super(S2MC2LossCell, self).__init__()
...
class S2MC2WithoutLossScaleCell(nn.Cell):
    """
    Encapsulation class of S2MC2 training.
    ...
    def __init__(self, network, optimizer):
        super(S2MC2WithoutLossScaleCell, self).__init__(auto_prefix=False)
...
class S2MC2GPULossScaleCell(nn.Cell):
    """
    Encapsulation class of S2MC2 training.
    ...
    def __init__(self, network, optimizer, sens=1):
        super(S2MC2GPULossScaleCell, self).__init__(auto_prefix=False)
        self.image = ImagePreProcess()
        self.network = network
        self.network.set_grad()
        self.weights = optimizer.parameters
...
class S2MC2WithLossScaleCell(nn.Cell):
    """
    Encapsulation class of S2MC2 training.
    ...
    """
    def __init__(self, network, optimizer, sens=1):
        super(S2MC2WithLossScaleCell, self).__init__(auto_prefix=False)
        self.image = ImagePreProcess()
        self.network = network
        self.network.set_grad()
        self.weights = optimizer.parameters
...
class S2MC2Eval(nn.Cell):
    """
    Encapsulation class of S2MC2 testing.
